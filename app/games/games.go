...
func GetScores(c *gin.Context) {
	...
	endpoint := fmt.Sprintf("%s/scoreboardV2?DayOffset=0&LeagueID=00&gameDate=%s", c.GetString("NBA_ENDPOINT"), today)
	...
	for _, set := range scoreboard.ResultSets {
		if set.Name == "LineScore" {
			for i, row := range set.RowSet {
				...
				var homeTeam Team
				var awayTeam Team
				homeTeam.populateInfo(&boxScore, row, set.Headers)
				awayTeam.populateInfo(&boxScore, set.RowSet[i+1], set.Headers)
